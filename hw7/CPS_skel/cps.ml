...
let rec cps' exp = 
  ...
  | Ifp (e1, e2, e3) -> 
    ...
    Fn (k,
        App (cps' e1, 
            Fn (v1, 
                Ifp (Var v1, App (cps' e2, Fn (v2, App (Var k, Var v2))), App (cps' e3, Fn (v3, App (Var k, Var v3))))
                )
            ) 
        )
  | Add (e1, e2) ->
  ...
  | Fst e ->  
    let p = new_name () in
    Fn (k, App (cps' e, Fn (p, App (Var k, Fst (Var p)))) )
  | Snd e ->  
    let p = new_name () in
    Fn (k, App (cps' e, Fn (p, App (Var k, Snd (Var p)))) )
