  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
       ...
       * let _ = ... 
       *)
      (* from env, find all locations reachable *)
      let filter_e e = List.filter
      (fun (_, ev) -> 
        match ev with
        | (Loc l) -> (List.exists (fun (loc, v) -> fst l == fst loc) m)
        | _ -> false)
      e in
      (* append all reachable locations to reachable_locs *)
      let do_gc e = List.iter 
      (fun (_, ev) ->
        match ev with
        | (Loc l) -> 
          (match List.filter (fun (loc, v) -> fst loc == fst l) m with
          | [] -> ()
          | li -> (List.iter
                  (fun (l, v) -> 
                    match v with 
                    (* Value is a record -> collects all locs of the record *)
                    | R r -> (let _ = reachable_locs := !reachable_locs@[l] in
                    List.iter (fun (x, l2) -> reachable_locs := !reachable_locs@[l2]) r)
                    (* else *)
                    | _ -> reachable_locs := !reachable_locs@[l])
                  li))
        | (Proc (x, c, e')) -> ())
      e in

      let filtered_e = filter_e e in
      let _ = do_gc filtered_e in
      let _ = List.iter (fun (_, e') -> let filtered_ke = filter_e e' in do_gc filtered_ke) k in
      (* don't need?
      (* from stack, find all locations reachable *)
      let _ = List.iter
      (fun sv -> 
        match sv with
        | V (L l) -> reachable_locs := !reachable_locs@[l]
        | V (R r) -> List.iter (fun (x, l) -> reachable_locs := !reachable_locs@[l]) r
        | M (x, Loc l) -> reachable_locs := !reachable_locs@[l]
        | _ -> ()
        )
      s in
      *)
